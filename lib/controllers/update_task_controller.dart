...
class UpdateTaskController extends GetxController{
  ...
  Future<bool>updateData({required int id, required String title,required String taskDetails,required String taskProcess}) async{
    try{
      _isLoading = true;
      update();
      ...
      final response = await db.update('taskTable',{'title':title,'taskDetails':taskDetails,'taskProcess': taskProcess},where: 'id = ?',whereArgs: [id]);
